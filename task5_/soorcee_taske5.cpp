...
int32_t main()
{
    ...
    case 0:
        ...
        std::cout << CountElements(arr_char, size);//для 4 и 5 задания не должно работать, тк char
        break;
    case 1:
        EnterTheArray(arr_int, size);
        OutputTheArray(arr_int, size);
        std::cout << CountElements(arr_int, size);
        std::cout << MaxElement(arr_int, size, maxIndex);//измени
        std::cout << MinElement(arr_int, size, minIndex);//измени 
    ...
    case 2:
        ...
        std::cout << MaxElement(arr_double, size, maxIndex);//измени
        std::cout << MinElement(arr_double, size, minIndex);//измени
